...
%{__rm} -rf %{buildroot}
%{__make} install install-init install-cgis install-html install-config install-commandmode install-exfoliation\
    DESTDIR="%{buildroot}" \
    INSTALL_OPTS="" \
    COMMAND_OPTS="" \
    INIT_OPTS=""

%{__rm} -rf %{buildroot}%{_sysconfdir}/nagios-server/objects/*.cfg
...
%{__perl} -pi -e '
   ...
   ' %{buildroot}%{_sysconfdir}/nagios-server/nagios.cfg
...
%attr(0755,nagios,nagios) %dir %{_sysconfdir}/nagios-server/
%attr(0644,nagios,nagios) %config(noreplace) %{_sysconfdir}/nagios-server/*.cfg
%attr(0755,nagios,nagios) %{_sysconfdir}/nagios-server/objects/
